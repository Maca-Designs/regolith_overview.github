...
const barTag = 'maca_templ:is_bar'

world.afterEvents.playerPlaceBlock.subscribe((data) => {
    const block = data.block
    Bar_Manager.updateBarsAround(block);
});
...
export class Bar_Manager {
    static update_Bar_States(Bar) {
        ...
        for (const blockData of blocks) {
            if (blockData.block != undefined) {
                if (!(blockData.block.isLiquid || blockData.block.isAir || ignoreList.has(blockData.block.typeId) || blockData.block.hasTag("maca_templ:no_connect") || blockData.block.typeId.includes("button") || ignoreList.has(blockData.block.typeId))) {
                    Bar.setPermutation(Bar.permutation.withState("maca_templ:connect_" + blockData.side, true));
                    if (blockData.block.hasTag("maca_templ:is_bar")) {
                        Bar.setPermutation(Bar.permutation.withState("maca_templ:neighbour", true));
                        blockData.block.setPermutation(blockData.block.permutation.withState("maca_templ:neighbour", true));
                    }
                } else {
                    Bar.setPermutation(Bar.permutation.withState("maca_templ:connect_" + blockData.side, false));
                }
            } else {
                Bar.setPermutation(Bar.permutation.withState("maca_templ:connect_" + blockData.side, false));
    ...
    static updateBarsAround(Block) {
        ...
        for (const block of blocks) {
            if (block != undefined) {
                if (block.hasTag(barTag)) {
                    this.update_Bar_States(block);
                }
